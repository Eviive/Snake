...
import { defaultGameSettings, Direction, SnakeSpriteType, Tile } from "../types/game.js";
...
export class Game {
    ...
    #treated = true;
    #gameEvents = []; // need to be removed after leaving the game
    #levelEvents = []; // need to be removed after the level stops
    onGameReady;
    onGameWin;
    onGameOver;
    constructor(level, sprite, onGameReady, onGameWin, onGameOver) {
        if (!Game.#isBuilding) {
            throw new Error("Game can only be built using the builder method");
        }
        const bgCanvas = document.querySelector("canvas#background-canvas");
        const fgCanvas = document.querySelector("canvas#foreground-canvas");
    ...
    static async builder(levelId, onGameReady, onGameWin, onGameOver) {
    ...
    #drawMap(delta = 0, iterating = true, dead = false) {
        const { dimensions: [width, height] } = this.#level;
